...
public class HuntParallel {
    ...
    private DungeonMapParallel dungeon;    // CHANGED: DungeonMap -> DungeonMapParallel
    ...
    public HuntParallel(int id, int pos_row, int pos_col, DungeonMapParallel dungeon) {  // CHANGED
        this.id = id;
        this.posRow = pos_row; // randomly allocated
        this.posCol = pos_col; // randomly allocated
        this.dungeon = dungeon;  // CHANGED
        this.stopped = false;
    }
    ...
    public int findManaPeak() {
        ...
        while(!dungeon.visited(posRow, posCol)) { // stop when hit existing path
            ...
            // REMOVED: if(DungeonHunterParallel.DEBUG) - HuntParallel shouldn't access DungeonHunterParallel
            switch(next) {
